...
class IntroIntro(ThreeDScene):
    def construct(self):
        world_x = Arrow3D(start=ORIGIN, end=RIGHT).set_color(RED)
        ...
        xcon1 = Cylinder(radius=.04, height=.1, direction=UP)
        xcon2 = Cylinder(radius=.04, height=.1, direction=UP)
        ...
        gimbal = VGroup(torusX, torusY, torusZ)
        core = VGroup(localX, localY, localZ).set_opacity(1.0)
        self.add(core)
